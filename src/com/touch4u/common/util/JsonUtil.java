...
public class JsonUtil
{
            ...
            try
            {
                        ...
                        && method.getParameterTypes().length == 0)
                {
                    ...
                    else if (Date.class.isAssignableFrom(obj.getClass()))
                    {
                        ...
                        int flag = time.indexOf("00:00:00");
                        if (flag == -1)
                            json.put(key, TimeUtil.formatDateTime((Date) obj));
                        else
                            json.put(key, TimeUtil.formatDate((Date) obj));
                    }
            ...
            try
            {
                ...
                if (name.startsWith("set"))
                {
                            ...
                            && types.length == 1)
                    {
                        ...
                        // 仅处理 String, Number, Boolean, Date 这几种类型
                        if (json.has(key)
                                && (clazz.isPrimitive() || clazz.equals(String.class) || clazz.equals(Boolean.class)
                                        || Number.class.isAssignableFrom(clazz) || Date.class.isAssignableFrom(clazz)))
                        {
                            ...
                            else
                            {
                                ...
                                else if (!clazz.equals(obj.getClass()))
                                {
                                    ...
                                    obj = clazz.getConstructor(new Class[] { String.class }).newInstance(
                                        new Object[] { tmp });
                                }
                            }
            ...
            try
            {
                ...
                for (Method m : ms)
                {
                    if (m.getName().startsWith("get") && m.getParameterTypes().length == 0
                            && !m.getName().equals("get") && !m.getName().equals("getClass"))
                    {
     ...
     * @return
     */
     public static JSONObject listMapToJsonObject(List<Map> list,String key){
